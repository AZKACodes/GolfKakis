...
class BookingDetailPdfService {
  const BookingDetailPdfService._();

  static Future<Uint8List> buildReceiptPdf({
    required BookingModel booking,
  }) async {
    final document = pw.Document();
    final currency = booking.currency ?? 'MYR';
                ...
                children: [
                  ...
                  _buildSection(
                    ...
                    children: [
                      ...
                      _buildInfoCard(label: 'Date', value: booking.dateLabel),
                      _buildInfoCard(
                        label: 'Tee Time',
                        value: booking.teeTimeSlot,
                      ),
                      _buildSummaryGrid(
                        items: [
                          ('Players', '${booking.playerCount}'),
                          ('Holes', _resolveHoleCount(booking.playType)),
                          ('Caddies', '${booking.caddieCount}'),
                          ('Buggy', '${booking.golfCartCount}'),
                        ],
                      ),
                    ],
                  ),
                  ...
                  _buildSection(
                    ...
                    children: [
                      ...
                      if ((booking.greenFeeTotal ?? 0) > 0)
                        _buildPriceRow(
                          label: 'Green Fee',
                          value: _formatPrice(booking.greenFeeTotal!, currency),
                        ),
                      ...
                      if ((booking.caddieTotal ?? 0) > 0)
                        _buildPriceRow(
                          label: 'Caddie',
                          value: _formatPrice(booking.caddieTotal!, currency),
                        ),
                      ...
                      if ((booking.sstTotal ?? 0) > 0)
                        _buildPriceRow(
                          label: 'SST',
                          value: _formatPrice(booking.sstTotal!, currency),
                        ),
                      if ((booking.discountAmount ?? 0) > 0)
                        _buildPriceRow(
                          label: 'Discount',
                          value:
                              '- ${_formatPrice(booking.discountAmount!, currency)}',
                        ),
                      _buildInfoCard(label: 'Total', value: booking.feeLabel),
                    ],
                  ),
                ],
              ),
  ...
  static pw.Widget _buildSummaryGrid({required List<(String, String)> items}) {
    return pw.Container(
      width: double.infinity,
      padding: const pw.EdgeInsets.all(12),
      decoration: pw.BoxDecoration(
  ...
  static String _resolveHoleCount(String? playType) {
    return playType?.trim().toLowerCase() == '18_holes' ? '18' : '9';
  }
